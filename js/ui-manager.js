    ...
    if (selectedTowers.length > 0) {
        ...
        if (selectedTower) {
            ...
            if (['MIND', 'ORBIT', 'CAT'].includes(selectedTower.type)) {
                const toggleModeBtn = uiElements.toggleModeBtn;
                if (toggleModeBtn) {
                    toggleModeBtn.classList.remove('hidden');
                    // Reset classes
                    toggleModeBtn.classList.remove('bg-purple-800', 'border-purple-500', 'shadow-[0_4px_0_#6b21a8]', 'bg-cyan-700', 'border-cyan-500', 'shadow-[0_4px_0_#0e7490]');

                    if (selectedTower.type === 'ORBIT') {
                        toggleModeBtn.classList.add('bg-cyan-700', 'border-cyan-500', 'shadow-[0_4px_0_#0e7490]');
                    } else {
                        toggleModeBtn.classList.add('bg-purple-800', 'border-purple-500', 'shadow-[0_4px_0_#6b21a8]');
                    }
                }
...
function updateTargetingButton(targetingMode, towerType) {
    ...
    if (targetingMode === 'mixed') {
        ...
        switch (targetingMode) {
            case 'strongest':
                uiElements.toggleTargetingBtn.classList.add('bg-red-800', 'border-red-400', 'shadow-[0_4px_0_#991b1b]', 'target-strongest-btn');
                break;
            case 'weakest':
                uiElements.toggleTargetingBtn.classList.add('bg-yellow-400', 'border-yellow-300', 'shadow-[0_4px_0_#ca8a04]', 'text-black', 'target-weakest-btn');
                break;
            case 'furthest':
                uiElements.toggleTargetingBtn.classList.add('bg-blue-800', 'border-blue-400', 'shadow-[0_4px_0_#1e40af]', 'text-white', 'target-furthest-btn');
                break;
        }
    }
}
