...
export class Projectile {
    ...
    update(onHit, enemies, effects, deltaTime) {
        ...
        const hitCondition = distance < moveDistance || (this.target && Math.hypot(this.x - this.target.x, this.y - this.target.y) < this.target.size);
        ...
        if (hitCondition) {
            ...
            // NINE_PIN projectiles are piercing and only get destroyed when they reach their target's original location.
            if (this.owner.type === 'NINE_PIN') {
                return false;
            }

            return this.owner.type === 'NINE_PIN';
        } else {
