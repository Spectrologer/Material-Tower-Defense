...
test('Tower should preserve merge and gold multipliers', () => {
    const tower = new Tower(200, 300, 'CASTLE');
    tower.damageMultiplierFromMerge = 1.5;
    tower.goldBonusMultiplier = 2.0;
    
    const json = tower.toJSON();
    const restored = Tower.fromJSON(json);
    
    assert.equal(restored.damageMultiplierFromMerge, 1.5);
    assert.equal(restored.goldBonusMultiplier, 2.0);
});
...
test('ENT tower should preserve aura properties', () => {
    ...
    assert.equal(restored.type, 'ENT');
    assert.equal(restored.mode, 'slow');
    assert.equal(restored.attackSpeedBoost, tower.attackSpeedBoost);
    assert.equal(restored.damageBoost, tower.damageBoost);
    assert.equal(restored.enemySlow, tower.enemySlow);
});

test('CAT tower should preserve gold bonus and aura properties', () => {
    const tower = new Tower(250, 250, 'CAT');
    tower.mode = 'boost';
    tower.goldBonusMultiplier = 1.5;
    
    const json = tower.toJSON();
    const restored = Tower.fromJSON(json);
    
    assert.equal(restored.type, 'CAT');
    assert.equal(restored.mode, 'boost');
    assert.equal(restored.goldBonus, tower.goldBonus);
    assert.equal(restored.goldBonusMultiplier, 1.5);
    assert.equal(restored.attackSpeedBoost, tower.attackSpeedBoost);
});
